...
public class BPNetwork extends NeuralNetwork {
	private static final double LEARN_RATE = 0.001;
    ...
    //static Logger logger = Logger.getLogger(BpDemo.class);  
	/**
	 * 训练模型
	 */
	@Override
	 ...
	 */
	@Override
	public double predict(Dataset testSet, String outName) {
		double errorrate=0;
		try {
			...
			if(encodeLable)
			{
			    ...
			    errorrate=rightCount * 1.0 / testSet.size();
			}
			else if (hasLable)
			{
				//Log.i("error rate ", "" + (sum / testSet.size()));
				//logger.warn("error rate"+sum / testSet.size()); 
				errorrate=sum / testSet.size();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public double test(Dataset dataSet) {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				// 更新后面各层的输出值
				updateOutput();
				double[] output = getLayer(layerNum - 1).getOutputs();//获取最后一层的输出值
				if (!encodeLable) {
					sum += Math.abs(output[0] - record.getLable());
